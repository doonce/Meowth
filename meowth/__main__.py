...
async def _get_generic_counters(guild, pkmn, weather=None):
    ...
    def clean(txt):
        return txt.replace('_', ' ').title()
    title = _('{form}{pkmn} | {weather} | Unknown Moveset').format(form=f"{form.replace('_', ' ').title()} ", pkmn=pkmn.title(), weather=weather_list[index].title())
    stats_msg = _("**CP:** {raid_cp}\n").format(raid_cp=raid_cp)
    stats_msg += _("**Weather:** {weather}\n").format(weather=clean(weather))
    stats_msg += _("**Attacker Level:** {atk_levels}").format(atk_levels=atk_levels)
    ...
    for moveset in data['byMove']:
        ...
        ctrs = moveset['defenders'][-6:]
        title = _('{form}{pkmn} | {weather} | {movesetstr}').format(form=form.title(), pkmn=pkmn.title(), weather=weather_list[index].title(), movesetstr=movesetstr)
        ctrs_embed = discord.Embed(colour=guild.me.colour)
        ctrs_embed.set_author(name=title, url=title_url, icon_url=hyperlink_icon)
        ctrs_embed.set_thumbnail(url=img_url)
        ctrs_embed.set_footer(text=_('Results courtesy of Pokebattler'), icon_url=pbtlr_icon)
...
async def _here(channel, author, count, party, entered_interest=None, boss_list=None):
    ...
    if lobby:
        if author.id in lobby.get('trainers', []):
            try:
                guild_dict[channel.guild.id]['raidchannel_dict'][channel.id]['lobby']['trainers'].remove(author.id)
...
async def _lobby(message, count):
    ...
    guild_dict[message.guild.id]['raidchannel_dict'][message.channel.id]['lobby']['trainers'].append(message.author.id)
...
async def starting(ctx, team: str = ''):
    ...
    if timeout or res.emoji == config['answer_no']:
        ...
        if starttime:
            ...
            timestr = ' '
        guild_dict[ctx.guild.id]['raidchannel_dict'][ctx.channel.id]['lobby'] = {"exp":time.time() + 120, "team":team, "trainers":id_startinglist, "starting_dict":starting_dict}
...
async def backout(ctx):
    ...
    if battle_lobby:
        ...
        await channel.send(_('Backout - Meowth! {author} has requested that the group consisting of {lobby_list} and the people with them to back out of the battle! Since you were already battling, please reply with **!here** again before you start.').format(author=author.mention, lobby_list=', '.join(lobby_list)))
