...
async def gym_match_prompt(channel, author_id, gym_name, gyms):
    ...
    if score < 80:
        try:
            ...
            reaction, __ = await utils.ask(Meowth, q_msg, author_id)
        except TypeError:
            await q_msg.delete()
            return None
        if not reaction:
...
async def profile(ctx, user: discord.Member = None):
    ...
    embed.add_field(name=_("Trainer Code"), value=f"{guild_dict[ctx.guild.id]['trainers'].setdefault(user.id,{}).get('trainercode',None)}", inline=True)
    ...
    embed.add_field(name=_("Nest Reports"), value=f"{guild_dict[ctx.guild.id]['trainers'].setdefault(user.id,{}).get('nest_reports',0)}", inline=True)
    await ctx.send(embed=embed)
    ...
    for trainer in leaderboard:
        ...
        if user:
            ...
            if guild_dict[ctx.guild.id]['configure_dict']['nest']['enabled']:
                field_value += _("Nests: **{nest}** | ").format(nest=trainer['nests'])
...
async def counters(ctx, *, args = None):
    ...
    if checks.check_raidchannel(ctx) and not checks.check_meetupchannel(ctx):
        ...
        if pkmn:
            ...
            form = guild_dict[guild.id]['raidchannel_dict'][channel.id].get('form', None)
        else:
...
async def _counters(ctx, pkmn, user = None, weather = None, form = None, movesetstr = "Unknown Moveset"):
    ...
    async with ctx.typing():
        ...
        if form == "_ALOLA_FORM":
            ...
            form_url = ""
        img_url = 'https://raw.githubusercontent.com/doonce/Meowth/Rewrite/images/pkmn/{number}_{form}.png?cache=4'.format(number=str(get_number(pkmn)).zfill(3), form=form_url)
    ...
    if ctx.invoked_subcommand == None:
        ...
        if (checks.check_raidreport(ctx) or checks.check_exraidreport(ctx)):
            ...
            if raid_dict:
                ...
                for (r, e) in sorted(raid_dict.items(), key=itemgetter(1)):
                    if len(listmsg) < 5800:
                        ...
                        await channel.send(embed=discord.Embed(colour=ctx.guild.me.colour, description=listmsg))
            ...
            if activeraidnum == 0:
                ...
                list_message = await channel.send(embed=discord.Embed(colour=ctx.guild.me.colour, description=listmsg))
