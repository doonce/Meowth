        ...
        if message.embeds:
            if len(message.embeds[0]['title'].split(" ")) == 5 and server_dict[message.server.id]['autoraid']:
                    ...
                    try:
                        if server_dict[message.server.id]['raidchannel_dict'][channelid]['gymhuntrgps'] == ghgps:
                            ...
                            if server_dict[message.server.id]['raidchannel_dict'][channelid]['type'] == 'egg':
                                await _eggtoraid(ghpokeid.lower().strip(), channel, message.author, huntr)
                ...
                if ghduplicate == False and int(ghraidlevel) in server_dict[message.server.id]['raidlvls']:
                    ...
                    raid = discord.utils.get(message.server.roles, name = ghpokeid.lower().strip())
                    if raid is None:
                        try:
                            raid = await Meowth.create_role(server = message.server, name = ghpokeid.lower().strip(), hoist = False, mentionable = True)
                            raid = raid.mention
                    ...
                    raid_img_url = "https://raw.githubusercontent.com/doonce/Meowth/master/images/pkmn/{0}_.png?cache=3".format(str(raid_number).zfill(3))
                ...
                if ghduplicate == False and int(ghegglevel) in server_dict[message.server.id]['egglvls']:
                    ...
                    raid_embed.set_thumbnail(url=_("https://raw.githubusercontent.com/doonce/Meowth/master/images/eggs/{}.png?cache=3".format(str(ghegglevel))))
                    raid_embed.set_footer(text=_("Reported by @{author} - {timestamp}").format(author=message.author.display_name, timestamp=timestamp), icon_url=_("https://cdn.discordapp.com/avatars/{user.id}/{user.avatar}.{format}?size={size}".format(user=message.author, format="jpg", size=32)))
...
async def want(ctx):
    ...
    if len(want_list) == 1 and (len(added_list) == 1 or len(spellcheck_dict) == 1 or len(already_want_list) == 1):
        if len(added_list) == 1:
            ...
            #want_img_url = "https://raw.githubusercontent.com/doonce/Meowth/master/images/pkmn/{0}_.png?cache=3".format(str(want_number).zfill(3)) #This part embeds the sprite
...
async def _wild(message, huntr):
    ...
    if not huntr:
        ...
        if message.author.avatar:
            ...
            wild_embed.set_footer(text=_("Reported by @{author} - {timestamp}").format(author=message.author.display_name, timestamp=timestamp), icon_url=message.author.default_avatar_url)
        wild_embed.set_thumbnail(url=wild_img_url)
        wildreportmsg = await Meowth.send_message(message.channel, content=_("Meowth! Wild {pokemon} reported by {member}! Details: <https://pokehuntr.com/#{location_details}>").format(pokemon=wild, member=message.author.mention, location_details=wild_details, gps=wild_details),embed=wild_embed)
    ...
    if fromegg is True:
        ...
        if raid_split[0].lower() == 'assume':
            ...
            if server_dict[message.channel.server.id]['raidchannel_dict'][message.channel.id]['active'] == False:
                await _eggtoraid(raid_split[1].lower(), message.channel, message.author, huntr=False)
                return
            else:
            ...
            if server_dict[message.channel.server.id]['raidchannel_dict'][message.channel.id]['active'] == False:
                await _eggtoraid(" ".join(raid_split).lower(), message.channel, message.author, huntr=False)
                return
            else:
...
Message **!starting** when the raid is beginning to clear the raid's 'here' list.""").format(member= raid_messageauthor.mention, citychannel=reportcitychannel.mention if reportcitychannel else "UNKNOWN", pokemon=entered_raid.title(), location_details=egg_address)
    elif egglevel == "EX":
        hatchtype = "exraid"
...
Message **!starting** when the raid is beginning to clear the raid's 'here' list.""").format(pokemon=entered_raid.title(), member=raid_messageauthor.mention, citychannel=reportcitychannel.mention if reportcitychannel else "UNKNOWN", location_details=egg_address)
    raid_match = True if entered_raid in get_raidlist() else False
    if not raid_match:
        await Meowth.send_message(raid_channel, _("Meowth! The Pokemon {pokemon} does not appear in raids!").format(pokemon=entered_raid.title()))
        return
    ...
    if 'https://cdn.discordapp.com' in ctx.message.attachments[0]['url']:
        if 'png' in ctx.message.attachments[0]['url'].lower() or 'jpg' in ctx.message.attachments[0]['url'].lower():
            ...
            if 'EX Raid Battle' in txt or "This is a reward" in txt or "Please visit the Gym" in txt:
                ...
                if exraidcount > 0:
                    await Meowth.send_message(ctx.message.channel, "Meowth! {0}, it looks like you've got an EX Raid invitation! The following {1} EX Raids have been reported: \n {2} \n Reply with **the number** (1, 2, etc) of the EX Raid you have been invited to. If none of them match your invite, type 'N' and report it with **!exraid**".format(ctx.message.author.mention, str(exraidcount), exraidlist))
