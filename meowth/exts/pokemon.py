...
class Pokedex(commands.Cog):
    ...
    async def generate_lists(bot):
        ...
        for k, v in bot.pkmn_info.items():
            for form in v['forms']:
                ...
                if v['forms'][form].get('ditto'):
                    ditto_list.append(v['number'])
                if v['forms'][form].get('mega'):
                    mega_list.append(v['number'])
            if len(k.split()) > 1:
                for word in k.split():
    ...
    async def pkmn_json(self, ctx, *, pokemon: Pokemon=None):
        ...
        if error:
            ...
            if pkmn_ditto:
                pkmn_info[pokemon.name.lower()]['forms'][pkmn_form]['ditto'] = pkmn_ditto
            else:
                try:
