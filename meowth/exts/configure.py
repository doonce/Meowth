...
class Configure(commands.Cog):
    ...
    async def _configure_invite(self, ctx):
        ...
        if not config_dict_temp['exraid']['enabled']:
            return ctx
        await ctx.configure_channel.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description=_('Do you want access to EX raids controlled through members using the **!exinvite** command?\nIf enabled, members will have read-only permissions for all EX Raids until they use **!exinvite** to gain access. If disabled, EX Raids will inherit the permissions from their reporting channels.\n\nRespond with: **N** to disable, or **Y** to enable:')).set_author(name=_('Invite Configuration'), icon_url=self.bot.user.avatar_url))
