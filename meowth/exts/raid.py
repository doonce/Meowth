...
class Raid(commands.Cog):
    def __init__(self, bot):
        ...
        self.bot.channel_report_dicts = ['raidchannel_dict', 'meetup_dict', 'raidtrain_dict']
    ...
    async def on_raw_reaction_add(self, payload):
        ...
        elif str(payload.emoji) == self.bot.custom_emoji.get('raid_report', '\U0001F4E2') or str(payload.emoji) == self.bot.custom_emoji.get('meetup_report', '\U0001F4E2'):
            ...
            for boss in self.bot.raid_info['raid_eggs'][str(level)]['pokemon']:
                ...
                if boss and boss.id == pokemon.id:
                    if sum(boss.name.title() in s for s in self.bot.raid_info['raid_eggs'][str(level)]['pokemon']) > 1:
                        ...
                        return await message.channel.send(f"Meowth! **{boss.name.title()}** has multiple forms at level {str(level)}! Please try again with the form name so I know which one to use! Please choose from: **{(', ').join(form_list)}**", delete_after=10)
                    pokemon = boss
                    entered_raid = boss.name.lower()
                    break
        ...
        if fromegg == True:
            ...
            if raid_split[0].lower() == 'assume':
                if self.bot.config.allow_assume[egg_level] == 'False':
                    return await message.channel.send(_('Meowth! **!raid assume** is not allowed in this level egg.'), delete_after=10)
                if self.bot.guild_dict[message.channel.guild.id]['raidchannel_dict'][message.channel.id]['active'] == False:
                    return await self._eggtoraid(str(pokemon), message.channel, message.author)
                else:
                    return await self._eggassume(ctx, " ".join(raid_split), message.channel, message.author)
            ...
            for boss in self.bot.raid_info['raid_eggs'][str(egg_level)]['pokemon']:
                ...
                if boss and boss.id == pokemon.id:
                    if sum(boss.name.title() in s for s in self.bot.raid_info['raid_eggs'][str(egg_level)]['pokemon']) > 1:
                        ...
                        form_list = [x.replace(boss.name.title(), '').strip() for x in form_list]
                        return await raid_channel.channel.send(f"Meowth! **{boss.name.title()}** has multiple forms at level {str(egg_level)}! Please try again with the form name so I know which one to use! Please choose from: **{(', ').join(form_list)}**", delete_after=10)
                    pokemon = boss
                    entered_raid = boss.name.lower()
                    break
    ...
    async def _meetup(self, ctx, location):
        ...
        here_reaction = self.bot.custom_emoji.get('raid_here', '\U0001F4CD')
        cancel_reaction = self.bot.custom_emoji.get('raid_cancel', '\u274C')
        report_emoji = self.bot.custom_emoji.get('meetup_report', '\U0001F4E2')
        list_emoji = self.bot.custom_emoji.get('list_emoji', '\U0001f5d2')
        react_list = [maybe_reaction, omw_reaction, here_reaction, cancel_reaction]
    ...
    async def timerset(self, ctx, *, timer):
        ...
        if checks.check_exraidchannel(ctx) or checks.check_meetupchannel(ctx) or checks.check_trainchannel(ctx):
            if checks.check_eggchannel(ctx) or checks.check_meetupchannel(ctx) or checks.check_trainchannel(ctx):
                now = datetime.datetime.utcnow() + datetime.timedelta(hours=self.bot.guild_dict[channel.guild.id]['configure_dict']['settings']['offset'])
                ...
                if message.author.id == guild.me.id or "Meowth" in message.author.display_name:
                    if (_('is interested') in message.content) or (_('on the way') in message.content) or (_('at the raid') in message.content) or (_('at the event') in message.content) or (_('no longer') in message.content) or (_('left the raid') in message.content):
                        if message.raw_mentions:
                            ...
                            continue
            if not report_dict:
                question = await ctx.channel.send(f"{ctx.author.mention} Is this a raid \u2694, a meetup \U0001F46A, or a train \U0001F682 channel?")
                try:
                    timeout = False
                    res, reactuser = await utils.ask(self.bot, question, ctx.author.id, react_list=['\u2694', '\U0001F46A', '\U0001F682'])
                except TypeError:
                    timeout = True
                if timeout:
                    report_dict = 'raidchannel_dict'
    ...
    async def _rsvp(self, ctx, rsvp_type, party_info):
        ...
        if rsvp_type == "maybe":
            ...
            self.bot.guild_dict[channel.guild.id]['raidchannel_dict'][channel.id]['lobby']['starting_dict'][author.id] = {"count":count, "status":trainer_dict['status'], "party":party}
        if entered_interest:
