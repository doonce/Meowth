...
class Tutorial:
    ...
    async def trade_tutorial(self, ctx, config):
        ...
        await ctx.tutorial_channel.send(
            f"This server utilizes the **{ctx.prefix}trade** command to "
            "list your pokemon up for trade. You can list one at a time using "
            f"command: **{ctx.prefix}trade <pokemon>**. You can also use forms "
            " of pokemon such as `alolan vulpix`, `unown y`, or `shiny absol`."
            f"\nEx: `{ctx.prefix}trade unown y`")
